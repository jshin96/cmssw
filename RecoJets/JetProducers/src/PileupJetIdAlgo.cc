...
PileupJetIdentifier PileupJetIdAlgo::computeIdVariables(const reco::Jet* jet,
  ...
  float dRmin(1000);
  float LeadcandPuppiWeight = 1.0;
  float SecondcandPuppiWeight = 1.0;
  float LeadNeutcandPuppiWeight = 1.0;
  float LeadEmcandPuppiWeight = 1.0;
  float LeadChcandPuppiWeight = 1.0;
  float TrailcandPuppiWeight = 1.0;


  for (unsigned i = 0; i < jet->numberOfSourceCandidatePtrs(); ++i) {
    reco::CandidatePtr pfJetConstituent = jet->sourceCandidatePtr(i);
//    reco::CandidatePtr weightpfJetConstituent = jet->sourceCandidatePtr(i);
  ...
  internalId_.leadPt_ = (lLead->pt())*LeadcandPuppiWeight;
  internalId_.leadEta_ = lLead->eta();
  internalId_.leadPhi_ = lLead->phi();

  if (lSecond != nullptr){
    internalId_.secondPt_ = (lSecond->pt())*SecondcandPuppiWeight;
  ...
  if (lLeadNeut != nullptr){
    internalId_.leadNeutPt_ = (lLeadNeut->pt())*LeadNeutcandPuppiWeight;
  ...
  if (lLeadEm != nullptr){
    internalId_.leadEmPt_ = (lLeadEm->pt())*LeadEmcandPuppiWeight;
  ...
  if (lLeadCh != nullptr){
    internalId_.leadChPt_ = (lLeadCh->pt())*LeadChcandPuppiWeight;
...
//    reco::CandidatePtr temp_weightpfJetConstituent = jet->sourceCandidatePtr(k);
   ...
   float partPuppiWeight = 1.0;
    ...
    if (usePuppi)
      partPuppiWeight = weights_[jet->sourceCandidatePtr(k)];
  ...
  internalId_.jetR_ = (lLead->pt())*LeadcandPuppiWeight / sumPt;
  if (lLeadCh != nullptr){
    internalId_.jetRchg_ = (lLeadCh->pt())*LeadChcandPuppiWeight / sumPt;
  } else { 
